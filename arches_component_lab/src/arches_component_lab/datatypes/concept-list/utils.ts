import type { CollectionItem, ConceptListValue } from "@/arches_component_lab/datatypes/concept-list/types.ts";
import { getOption, flattenCollectionItems } from "@/arches_component_lab/datatypes/concept/utils.ts";
...
export function convertSelectionToModelValue(
    ...
    const allSelectedOptions: CollectionItem[] = Object.keys(conceptOptionIds).map(
        (key) => getOption(key, options) as CollectionItem,
    );
